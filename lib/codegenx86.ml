...
let codegenx86_print _ = "\tmov rax, 1\n \tmov rdi, 1" |> add_string code
...
let print_word n = "\t print \" \", " ^ n ^ "\n" |> add_string code

(*constant build*)
let asm_word name n = "\t" ^ name ^ ": dq " ^ n ^ "\n" |> add_string data_declr
...
let codegenx86_prog prog =
  ...
  Array.iter
    (function
      | FunDef f when f.id = "main" -> found_main := true
      | FunDef f -> codegenx86_func (FunDef f))
    prog;
  if not !found_main then failwith "Codegenx86 requires a 'main' function."
